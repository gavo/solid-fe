...
const urlBase = import.meta.env.VITE_API_URL + "/producto";
const newProduct = {
	id: 0,
	nombre: "",
	nombreExtranjero: "",
	...
	const isProdForSale = (selected) => {
		...
		return (
			data.productos.reduce(
				(resp, current) =>
					resp + (selected.id === current.producto.id ? 1 : 0),
				0
			) === 0
		);
	};
	...
	return (
		<div className="big-container">
			...
			<input
				...
				id="filter-producto"
			/>
			<button onClick={(evt) => showFormProducto(newProduct)}>
				Agregar
			</button>
			<table>
				<thead>
						...
						.map((producto, key) => {
							return (
								<tr key={key}>
									...
									<td>
										<button
											onClick={() => showFormProducto(producto)}
										>
											Editar
										</button>
									</td>
