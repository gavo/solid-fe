...
const urlBase = import.meta.env.VITE_API_URL + "/fabricante";
...
const FabricantePage = () => {
	...
	const submitFabricante = async (id, nFabricante) => {
		...
		if (id) {
			...
			await axios
				...
				.then((obj) => {
					alert(
						"Fabricante " + obj.data.nombre + " actualizado exitosamente"
					);
				})
				.catch((error) => {
			...
			await axios
				...
				.then((obj) => {
					alert(
						"Fabricante " + obj.data.nombre + " Registrado exitosamente"
					);
				})
				.catch((error) => {
					alert(error.response.data);
	...
	return (
		<div className="container">
			...
			/>
			<button onClick={() => showFormFabricante(newFabricante)}>
				Agregar
			</button>
			<table>
				<thead>
						...
						.map((fabricante, key) => {
							return (
								<tr key={key}>
									...
									<td>
										<button
											onClick={() => showFormFabricante(fabricante)}
										>
											Editar
										</button>
									</td>
			...
			{modal && (
				<SimpleObjectForm
					title={
						(select.id == 0 ? "Registrar" : "Actualizar") + " Fabricante"
					}
